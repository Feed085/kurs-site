...
const AdminExamPanel = () => {
  ...
  const handlePublish = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedDraftIds.length === 0) {
      toast.error('Ən azı bir müəllim imtahanı seçin');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.success('İmtahan bütün tələbələr üçün hazırlandı');
        setSelectedDraftIds([]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-gray-900">Müəllim imtahanlarını idarə et</h1>
          <p className="mt-1 text-gray-500">Layihələri seçib yayımlayın, daha sonra həmin imtahanları keçmiş imtahanlar bölməsində izləyin və nəticələri açın.</p>
        </div>
        <button
          type="button"
          onClick={loadData}
      ...
      <div className="flex flex-wrap gap-3">
        {[
          ...
          { key: 'history', label: 'Keçmiş imtahanlar', count: publishedExams.length },
        ].map((tab) => {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-sm">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-gray-400">Gözləyən layihələr</div>
          <div className="mt-3 text-3xl font-black text-gray-900">{drafts.length}</div>
          <div className="mt-1 text-sm text-gray-500">Admin baxışı üçün gələn müəllim imtahanları</div>
        </div>
        <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-sm">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-gray-400">Seçilən suallar</div>
          <div className="mt-3 text-3xl font-black text-gray-900">{selectedQuestionCount}</div>
          <div className="mt-1 text-sm text-gray-500">Birləşdiriləcək sual sayı</div>
        </div>
        <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-sm">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-gray-400">Keçmiş imtahanlar</div>
          <div className="mt-3 text-3xl font-black text-gray-900">{publishedExams.length}</div>
          <div className="mt-1 text-sm text-gray-500">Yayımlanmış və arxivdə saxlanılan imtahanlar</div>
        </div>
      </div>
          ...
          <section className="space-y-5">
            <div className="rounded-[32px] border border-gray-100 bg-white p-4 shadow-sm sm:p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D4AF37]">Müəllim göndərişləri</p>
                  <h2 className="mt-2 text-2xl font-black text-gray-900">Seç və birləşdir</h2>
                </div>
                <div className="relative max-w-md flex-1">
                  ...
                  <input
                    ...
                    type="text"
                    placeholder="Başlıq və ya müəllim ilə axtar..."
            ...
            {loading ? (
              <div className="rounded-[32px] border border-gray-100 bg-white p-10 text-center text-gray-500 shadow-sm">İmtahan göndərişləri yüklənir...</div>
            ) : filteredDrafts.length === 0 ? (
              <div className="rounded-[32px] border border-dashed border-gray-200 bg-white p-10 text-center italic text-gray-400">Seçim üçün uyğun müəllim imtahanı tapılmadı.</div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                {filteredDrafts.map((draft) => {
                  const draftId = String(draft._id);
                  const isSelected = selectedDraftIds.includes(draftId);
                  const instructorName = `${draft.instructor?.name || ''} ${draft.instructor?.surname || ''}`.trim() || 'Naməlum müəllim';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
                        <span>{draft.instructor?.email || 'E-poçt yoxdur'}</span>
          ...
          <aside className="space-y-5">
            <form onSubmit={handlePublish} className="rounded-[32px] border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D4AF37]">İmtahan yarat</p>
                <h2 className="mt-2 text-2xl font-black text-gray-900">Yayımlama formu</h2>
              <p className="mt-1 text-sm text-gray-500">Bu bölmə yalnız müəllim layihələrini seçib yekun imtahan yaratmaq üçündür. Yayımlanmış imtahanlar keçmiş imtahanlar bölməsində saxlanılır.</p>

              <div className="mt-5 space-y-4">
                <div className="rounded-2xl bg-gray-50 px-4 py-3 text-sm font-bold text-gray-600">
                  {selectedDraftIds.length} müəllim imtahanı seçilib, {selectedQuestionCount} sual birləşdiriləcək.
                </div>

                <label className="block space-y-1.5">
                  <span className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-500 italic">İmtahan adı</span>
                  <input
                    required
                    ...
                    placeholder="Məsələn: Aprel yekun imtahanı"
                  />
                </label>

                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block space-y-1.5">
                    <span className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-500 italic">Müddət</span>
                    <input
                      required
                      min="1"
                  ...
                  <label className="block space-y-1.5">
                    <span className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-500 italic">Şifrə</span>
                    <input
                      required
                      ...
                      type="text"
                      className="w-full rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 text-sm font-bold outline-none transition-all focus:border-[#D4AF37] focus:bg-white"
                      placeholder="Məsələn: EXAM-2026"
                    />
                  </label>
                </div>

                <label className="block space-y-1.5">
                  <span className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-500 italic">Başlama vaxtı</span>
                  <input
                    required
                ...
                <label className="flex items-start gap-3 rounded-2xl border border-gray-100 bg-gray-50 p-4">
                  ...
                  />
                  <span>
                    <span className="block text-sm font-black text-gray-900">Tələbə panelində göstər</span>
                    <span className="mt-1 block text-xs text-gray-500">Aktivdirsə imtahan paneldə əvvəlcədən görünəcək, amma start vaxtına qədər açıla bilməyəcək.</span>
                  </span>
                </label>
              </div>
            ...
            <section className="rounded-[32px] border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
              ...
              <h2 className="mt-2 text-2xl font-black text-gray-900">Yayımlanan imtahanlar arxivə düşür</h2>
              <p className="mt-2 text-sm text-gray-500">İmtahan yayımlandıqdan sonra iş siyahısından çıxmır, ayrıca `Keçmiş imtahanlar` bölməsində saxlanılır. Bitmiş imtahana klik etdikdə iştirakçı və cavab detalları ayrıca səhifədə açılır.</p>
            </section>
          </aside>
        </div>
      ) : (
        <div className="space-y-6">
          <section className="rounded-[32px] border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D4AF37]">Hazırda aktiv imtahan</p>
                <h2 className="mt-2 text-2xl font-black text-gray-900">Aktiv kart</h2>
                <p className="mt-1 text-sm text-gray-500">Başlama vaxtı çatmış imtahan burada ayrıca görünür və şifrəsi bu brauzerdə saxlanılır.</p>
              </div>
              {activeExam ? (
                <button
                  type="button"
            ...
            {!activeExam ? (
              <div className="mt-5 rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-6 text-sm text-gray-500">Hazırda aktiv imtahan yoxdur. Planlaşdırılmış və bitmiş imtahanlar aşağıdakı siyahıda görünür.</div>
            ) : (
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-gray-600">
                      ...
                      <span className="rounded-full bg-white px-3 py-1">{activeExam.duration} dəq</span>
                  ...
                  <div className="grid w-full gap-3 sm:grid-cols-2 lg:max-w-md">
                    <div className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-gray-700">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-gray-400">Şifrə</div>
                      <div className="mt-2 inline-flex items-center gap-2 break-all text-sm text-gray-900 sm:text-base">
                        ...
                        {accessCodeMap[resolveEntityId(activeExam._id ?? activeExam.id)] || 'Bu brauzerdə saxlanmayıb'}
                      </div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-gray-700">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-gray-400">Nəticələr</div>
                      <div className="mt-2 text-base text-gray-900">{activeExam.resultsCount} iştirak</div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-gray-700">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-gray-400">Təyin olunan tələbə</div>
                      <div className="mt-2 text-base text-gray-900">{activeExam.assignedStudentsCount}</div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-gray-700">
                      <div className="text-[10px] uppercase tracking-[0.16em] text-gray-400">Gözləyən yoxlama</div>
          ...
          <section className="rounded-[32px] border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D4AF37]">Keçmiş imtahanlar</p>
                <h2 className="mt-2 text-2xl font-black text-gray-900">Yayımlanmış imtahan arxivi</h2>
                <p className="mt-1 text-sm text-gray-500">Burada yayımlanmış bütün imtahanları tarix və status üzrə süzə bilərsiniz.</p>
              </div>
            ...
            <div className="mt-6 grid gap-4 xl:grid-cols-2">
              {loading ? (
                <div className="xl:col-span-2 rounded-2xl border border-gray-100 bg-gray-50 p-10 text-center text-gray-500">Keçmiş imtahanlar yüklənir...</div>
              ) : filteredPublishedExams.length === 0 ? (
                <div className="xl:col-span-2 rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-10 text-center italic text-gray-400">Filtrə uyğun yayımlanmış imtahan tapılmadı.</div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-gray-600">
                        <div className="rounded-xl bg-gray-50 px-3 py-2 font-bold">{exam.duration} dəq</div>
                        <div className="rounded-xl bg-gray-50 px-3 py-2 font-bold">{exam.questions?.length || 0} sual</div>
                        <div className="inline-flex items-center gap-2 rounded-xl bg-gray-50 px-3 py-2 font-bold"><Users className="h-3.5 w-3.5" /> {exam.assignedStudentsCount} tələbə</div>
                        <div className="inline-flex items-center gap-2 rounded-xl bg-gray-50 px-3 py-2 font-bold"><FileText className="h-3.5 w-3.5" /> {exam.resultsCount} nəticə</div>
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 text-[11px] font-bold uppercase tracking-[0.12em] text-gray-500">
                        ...
                        <span className="rounded-full bg-gray-50 px-3 py-1">{exam.completedResultsCount} tamamlanmış nəticə</span>
                      ...
                      {status === 'completed' ? (
                        <div className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#A87A1F]">
                          Bitmiş imtahanın iştirakçılarını aç
                          <ArrowRight className="h-4 w-4" />
                        </div>
                      ) : (
                        <div className="mt-4 text-sm font-bold text-gray-500">İmtahanın ümumi vəziyyətini aç</div>
                      )}
                    </button>
                  );
                })

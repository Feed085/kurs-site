...
type CourseReviewFormProps = {
  ...
  onSubmitted?: (updatedCourse: any) => void;
};
...
export default function CourseReviewForm({
  ...
  }, [courseId, initialRating, initialComment]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(data.message || t('review.success.course_review_saved', { defaultValue: 'Rəyiniz qeyd edildi' }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-gray-100 bg-gray-50 p-6">
      <div>
        ...
        <h3 className="text-xl font-black text-gray-900">{t('courses.rate_course', { defaultValue: 'Bu kursu qiymətləndirin' })}</h3>
      </div>
      ...
      <Textarea
        placeholder={t('review.course_placeholder', { defaultValue: 'Kurs haqqında fikirlərinizi bölüşün...' })}
        value={comment}

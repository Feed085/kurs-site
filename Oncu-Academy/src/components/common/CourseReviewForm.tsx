...
export default function CourseReviewForm({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(data.message || 'Rəyiniz qeyd edildi');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-gray-100 bg-gray-50 p-6">
      <div>
        ...
        <h3 className="text-xl font-black text-gray-900">Bu kursu qiymətləndirin</h3>
      </div>
      ...
      <Textarea
        placeholder="Kurs haqqında fikirlərinizi bölüşün..."
        value={comment}
